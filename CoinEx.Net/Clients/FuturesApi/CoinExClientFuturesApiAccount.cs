...
namespace CoinEx.Net.Clients.FuturesApi
{
    ...
    public class CoinExClientFuturesApiAccount : ICoinExClientFuturesApiAccount
    {
        ...
        /// <inheritdoc />
        public async Task<WebCallResult<Dictionary<string, CoinExAssetQuery>>> AssetQueryAsync(CancellationToken ct = default)
        {
            var result = await _baseClient.Execute<Dictionary<string, CoinExAssetQuery>>(_baseClient.GetUrl(AssetQueryEndpoint), HttpMethod.Get, ct, null, true).ConfigureAwait(false);
            if (result)
            {
                foreach (var b in result.Data)
                    b.Value.Asset = b.Key;
