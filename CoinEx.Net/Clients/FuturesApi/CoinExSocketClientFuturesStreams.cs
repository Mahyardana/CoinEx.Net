...
{
    /// <inheritdoc cref="ICoinExSocketClientFuturesStreams" />
    public class CoinExSocketClientFuturesStreams : SocketApiClient, ICoinExSocketClientFuturesStreams
    {
        #region fields
        ...
        private const string OrderSubject = "position";
        ...
        /// </summary>
        public CoinExSocketClientFuturesStreams(Log log, CoinExSocketClient baseClient, CoinExSocketClientOptions options)
        ...
        /// <inheritdoc />
        public Task<CallResult<UpdateSubscription>> SubscribeToPositionUpdatesAsync(Action<DataEvent<CoinExSocketOrderUpdate>> onMessage, CancellationToken ct = default)
            => SubscribeToPositionUpdatesAsync(Array.Empty<string>(), onMessage, ct);

        /// <inheritdoc />
        public async Task<CallResult<UpdateSubscription>> SubscribeToPositionUpdatesAsync(IEnumerable<string> symbols, Action<DataEvent<CoinExSocketOrderUpdate>> onMessage, CancellationToken ct = default)
        {
            var internalHandler = new Action<DataEvent<JToken[]>>(data =>
            {
                if (data.Data.Length != 2)
            ...
            var request = new CoinExSocketRequest(_baseClient.NextIdInternal(), OrderSubject, QueryAction, symbols.ToArray());
